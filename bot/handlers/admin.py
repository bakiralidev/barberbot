...
async def admin_edit_work_hours_start(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    # Add 'all days' button
    kb.button(text="🗓 Barcha kunlar uchun", callback_data="edit_work_hours_all")
    for i, name in enumerate(weekdays):
        kb.button(text=name, callback_data=f"edit_work_hours_day_{i}")
    kb.button(text="⬅️ Orqaga", callback_data="adm_main_menu")
    kb.adjust(2)
    await callback.message.edit_text("Qaysi kun uchun ish vaqtini o'zgartirasiz?", reply_markup=kb.as_markup())

# --- All days work hours ---
@router.callback_query(F.data == "edit_work_hours_all")
async def admin_edit_work_hours_all_start(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    user = await ensure_admin_user(callback.from_user.id, session)
    if not can_access_admin_panel(user):
        await callback.answer("Kirish rad etildi", show_alert=True)
        return
    await state.set_state(AdminState.edit_schedule_start)
    await state.update_data(edit_work_hours_weekday="all")
    await callback.message.edit_text("Barcha kunlar uchun boshlanish vaqtini kiriting (masalan, 09:00):")
...
async def admin_edit_work_hours_end_time(message: Message, state: FSMContext, session: AsyncSession):
    try:
        ...
        if weekday == "all":
            # Set for all days
            for wd in range(7):
                await update_work_schedule_day(session, wd, False, start, t)
            await message.answer("✅ Barcha kunlar uchun ish vaqti saqlandi!")
        else:
...
@router.callback_query(F.data.startswith("edit_work_hours_day_"))
async def admin_edit_work_hours_day(callback: CallbackQuery, state: FSMContext, session: AsyncSession):
    user = await ensure_admin_user(callback.from_user.id, session)
    if not can_access_admin_panel(user):
        await callback.answer("Kirish rad etildi", show_alert=True)
        return
    weekday = int(callback.data.split("_")[-1])
    await state.update_data(edit_work_hours_weekday=weekday)
    # Get current times
    query = select(WorkSchedule).where(WorkSchedule.weekday == weekday)
    res = await session.execute(query)
    day = res.scalar_one_or_none()
    cur_start = day.start_time.strftime('%H:%M') if day else "09:00"
    cur_end = day.end_time.strftime('%H:%M') if day else "18:00"
    await state.set_state(AdminState.edit_schedule_start)
    await callback.message.edit_text(f"Joriy vaqt: {cur_start} - {cur_end}\nYangi boshlanish vaqtini kiriting (masalan, 09:00):")

@router.message(AdminState.edit_schedule_start)
async def admin_edit_work_hours_start_time(message: Message, state: FSMContext):
    # Validate time
    try:
        t = datetime.strptime(message.text.strip(), "%H:%M").time()
        await state.update_data(edit_work_hours_start=str(t))
